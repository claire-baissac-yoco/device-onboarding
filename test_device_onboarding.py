...
def get_dummy_device_info():
    ...
    crate_number = 1
    damage_rating = 0
    return serial_number, imei, box_number, crate_number, damage_rating
...
def update_device_to_given_state(database: mockDatabase, serial_number, state) -> mockDatabase:
    ...
    box_number, crate_number, damage_rating = get_dummy_device_info()[2:]
    snn, imsi = 1, 2
    mode = 1
    keys = gen_keys()
    func_params = ['', '', '', f'{box_number}, {crate_number}', f'{damage_rating}', f'{snn}, {imsi}', f'{mode}', 'keys, mode', '', '[1, 2, 3, 4, 5, 6]']
    last = False
...
def test_can_record_device_damage():
    serial_number  = get_dummy_device_info()[0]
    database = create_database_with_dummy_device()
    database = update_device_to_given_state(database, serial_number, DeviceState.package_info_recorded)
    database.set_device_damage_rating(serial_number, 0)
    assert database.get_device_by_serial_number(serial_number).get_damage_rating() == 0
    assert database.get_device_by_serial_number(serial_number).get_state() == DeviceState.damage_recorded

def test_cannot_proceed_if_device_damaged():
    with pytest.raises(InvalidOperationException):
        serial_number  = get_dummy_device_info()[0]
        database = create_database_with_dummy_device()
        database = update_device_to_given_state(database, serial_number, DeviceState.package_info_recorded)
        database.set_device_damage_rating(serial_number, 1)
        database.update_device_sim_card(serial_number, 1, 1)
    assert database.get_device_by_serial_number(serial_number).get_state() == DeviceState.damage_recorded
